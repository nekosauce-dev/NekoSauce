...
class LolibooruFetcher(sources.BaseFetcher):
    site_name = "Lolibooru"
    source = Source.objects.get(name="Lolibooru")

    def get_url(self, path: str) -> str:
        return f"https://lolibooru.moe{path}"
    ...
    def get_file_url(self, id: str) -> str:
        qs = Source.objects.filter(tags__overlap=[f"lolibooru:post:id:{id}"])
    ...
    def _get_new_sauce_from_response(self, post: dict) -> Sauce:
        site_urls = [
            f"https://lolibooru.moe/post/show/{post['id']}",
        ...
        sauce = Sauce(
            ...
            + [
                f"lolibooru:post:md5:{post['md5']}",
                f"lolibooru:post:rating:{post['rating']}",
            ]
            + [f"lolibooru:tag:name:{tag}" for tag in post["tags"].split(" ")]
            + (
                [f"lolibooru:author:name:{urllib.parse.quote_plus(post['author'])}"]
                if post["author"]
                else []
            ),
    ...
    def get_sauce(self, id: str) -> Sauce:
        qs = Sauce.objects.filter(tags__overlap=[f"lolibooru:post:id:{id}"])
        ...
        while True:
            for index, response in grequests.imap_enumerated(
                ...
                if response.status_code == 520:
                    print(
                        f"({range(start_from, last_lolibooru_sauce_id + 1, page_range)[index]}-{range(start_from, last_lolibooru_sauce_id + 1, page_range)[index + 1]}) ERROR 520! Skipping..."
                    )
                    continue
                ...
                for sauce in new_sauces:
                    if sauce.source_site_id == str(last_lolibooru_sauce_id):
                        return
...
class LolibooruDownloader(sources.BaseDownloader):
    fetcher = LolibooruFetcher
    site_name = "Lolibooru"

    def check_url(self, url: str) -> bool:
        return url.startswith("https://lolibooru.moe")
    ...
    def download_request(self, url: str):
        if urllib.parse.urlparse(url).path.startswith("/post/"):
            url = self.fetcher.get_file_url(LolibooruDownloader.get_sauce_id(url))
...
class LolibooruTagger(sources.BaseTagger):
    source = "lolibooru"
    source_domain = "lolibooru.moe"
    resources = ["post"]
