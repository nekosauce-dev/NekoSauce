...
class SafebooruFetcher(sources.BaseFetcher):
    site_name: str = "Safebooru"
    ...
    def get_sauce(self, id: str) -> Sauce:
        qs = Sauce.objects.filter(tags__overlap=[f"safebooru:post:id:{id}"])
    ...
    def get_new_sauce_from_response(self, post: dict) -> Sauce:
        site_urls = [
            f"https://safebooru.org/index.php?page=post&s=view&id={post['id']}"
        ] + (
        ...
        sauce = Sauce(
            site_urls=site_urls,
            api_urls=[
                f"https://safebooru.org/index.php?page=dapi&q=index&json=1&s=post&id={post['id']}"
            ],
            file_urls=[
    ...
    def get_sauce_from_response(self, post: dict) -> Sauce:
        sauce = Sauce.objects.filter(
            tags__overlap=[f"safebooru:post:id:{post['id']}"]
        ).first()
...
class SafebooruDownloader(sources.BaseDownloader):
    fetcher = SafebooruFetcher
    site_name = "Safebooru"

    def check_url(self, url: str) -> bool:
        return re.match(r"^https://([a-zA-Z0-9-]+\.)?safebooru\.org/", url) is not None
...
class SafebooruTagger(sources.BaseTagger):
    source = "safebooru"
    source_domain = "safebooru.org"
    resources = ["post", "tag"]
    ...
    def check_url(self, url: str) -> bool:
        try:
            ...
            return (
                url.startswith("https://safebooru.org")
