...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Statistic.objects.create(
            ...
            attribute="count",
            value=Sauce.objects.filter(
                source=Source.objects.get(name__iexact="danbooru")
            ).count(),
        )
        Statistic.objects.create(
            ...
            attribute="count",
            value=Sauce.objects.filter(
                source=Source.objects.get(name__iexact="gelbooru")
            ).count(),
        )
            ...
            for i in range(len(Hash.Algorithm)):
                Statistic.objects.create(
                    resource="hash:{}:{}".format(
                        hash_model.__name__.lower()[4:],
                        Hash.Algorithm._value2member_map_[i],
                    ),
                    attribute="count",
                    value=hash_model.objects.filter(
                        algorithm=Hash.Algorithm._value2member_map_[i]
                    ).count(),
                )
